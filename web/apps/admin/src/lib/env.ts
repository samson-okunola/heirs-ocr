/**
 * Server-side environment for the admin console.
 *
 ...
 * here, so anything this app needs must be in `web/apps/admin/.env.local`.
 *
...
const missingMessage = (names: readonly string[]): string =>
  ...
  `Copy web/apps/admin/.env.local.example to web/apps/admin/.env.local and fill it in.\n` +
